...
datafile=ssd-raid10.log 
out=ssd-raid10.jpg

wlist="w100-ssd w100-raid10-1 w100-raid10-2"
wlists=""
for bp in $wlist
do
    #每10秒输出一次结果，合并6次
    ...
    ../tpcc-output-analyze.sh  tpcc.${bp}.out > b2.${bp}.log
    wfiles="$wfiles b2.${bp}.log"
done
...
#set format y "%.2f"
#计算每秒的次数
plot datafile using 3:(\$1/$n) title "SSD 128G" with lines, \
    datafile using 6:(\$4/$n) title "SATA Raid1+0 2T(1)" with lines, \
    datafile using 9:(\$7/$n) title "SATA Raid1+0 2T(2)" with lines
